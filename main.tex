...
\BlankLine
\BlankLine
\KwIn{$\mathbf{X}\in\mathbb{R}^{N\times K}$, $\boldsymbol{\lambda}\in\mathbb{R}^{N}$,  $\mathbf{w}\in\mathbb{R}^{K}$, $\beta$, $\eta$, $\alpha$}    
\KwOut{$\mathbf{w}\in\mathbb{R}^{K}$}
\SetKwFunction{UpdatePredictor}{UpdatePredictor}
\SetKwProg{Fn}{Function}{:}{}
\BlankLine
\BlankLine
\Fn{\UpdatePredictor{$\mathbf{X}$, $\boldsymbol{\lambda}$, $\mathbf{w}$, $\beta$, $\eta$, $\alpha$}}{
	\BlankLine
    $\gamma\leftarrow\frac{1}{2\beta^2}$\;
   
	$\mathbf{y}\leftarrow  \Predict{$\mathbf{X}$,$\mathbf{w}$, $\alpha$}$\;
    $\mathbf{w}\leftarrow\mathbf{w}+\eta\gamma\alpha\sum_{i=1}^N{\mathbf{x_i}y_i(1-y_i)\left(y_i -  e^{-\gamma\lambda_i^2}\right)}$\;
...
\Begin{
    ...
    \tcp*[r]{\parbox[t]{3in}{\raggedleft Motor-control outputs related to a fixed grid 
    of points in the output space}}   
    \BlankLine
    
	\Repeat{$\frac{1}{K}\sum_{i=1}^K\boldsymbol{\xi} = 1$ }{

    	\BlankLine
	    \tcp*[r]{\parbox[t]{2in}{\raggedleft Update predictions over the grid positions in the output space}}  
        $\boldsymbol{\xi}\leftarrow\Predict{$\mathbf{\Psi_{grid}}$, $\mathbf{w}_p$, $\alpha$}$\;   
        $\boldsymbol{\omega}\leftarrow 1-\boldsymbol{\xi}$\;
		
    	\BlankLine
		
        \tcp*[r]{\parbox[t]{1.5in}{\raggedleft Sample goals and sample motor patterns from goals}}
        ...
        \tcp*[r]{\parbox[t]{3in}{\raggedleft Compute distances between goal representations and sensory encodings}}
        $\mathbf{E}\leftarrow\Int{$\mathbf{sout}$, $M$, $K$, $\beta$}$\;
        \For{$i=1$ \KwTo $N$}{
        	$\chi_i\leftarrow||\mathbf{g}_i - \mathbf{e}_i||$\tcp*[r]{
            \parbox[t]{3.in}{\raggedleft Match: distances between goals and 
            encoded states}}
        }\;
        
        \BlankLine
	    \tcp*[r]{\parbox[t]{1in}{\raggedleft Train SOMs and predictor}}
...
\Begin{
	...
	\Repeat{$\frac{1}{K}\sum_{i=1}^K\boldsymbol{\xi} = 1$ }{
       ...
       \BlankLine
    	...
    	$\mathbf{p_g}, \mathbf{A},\mathbf{G}\leftarrow$\,\GenerateFromMSOM{$\mathbf{W}_m$, $K$, $N$, $\boldsymbol{\gamma}$, $\beta$}\;  
        $\boldsymbol{\xi}_{g}\leftarrow\Predict{$\mathbf{G}$, $\mathbf{w}_p$, $\alpha$}$\;
        $\boldsymbol{\omega}_{g}\leftarrow 1 - \boldsymbol{\xi}_{g}$\;
        $\mathbf{A}\leftarrow\mathbf{A} + \mathcal{N}(\mathbf{0}, \nu\boldsymbol{\omega}_{g})$\; 
        $\mathbf{S}\leftarrow\mathbf{A}$\;
