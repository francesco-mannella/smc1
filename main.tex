...
\Fn{\UpdateSOM{$\mathbf{X}$, $\mathbf{W}$, $M$, 
	$K$, $\sigma$, $\eta$}}{

	\BlankLine
    
	$\gamma \leftarrow \frac{1}{2\sigma^2}$
    \tcp*[r]{Extension of the neighborhood}
    ...
    \For{$n\leftarrow0$ \KwTo {$N$}}{
        ...
        \mathbf{x}_n - \mathbf{w}_k ||$
        \tcp*[r]{Winning prototype}
       
		\BlankLine
                 
        \For{$k\leftarrow1$ \KwTo {$K$}}{
        	$\phi_{k, n}\leftarrow exp(\gamma(k - r_{n})^2)$
                 \tcp*[r]{\parbox[t]{2.2in}{\raggedleft 
                 Radial bases of distances from winner}}
                 
            $\mathbf{w}_k \leftarrow 
            \mathbf{w}_k + \eta
            \phi_{k, n}(\mathbf{x}_{n}-\mathbf{w}_{k})$
            \tcp*[r]{gradient descent}
        }
    }
    \Return{$\mathbf{W}$}
}
...
\KwIn{$\mathbf{X}\in\mathbb{R}^{N\times M}$, 
	$\mathbf{W}\in\mathbb{R}^{K\times M}$, $M$, $K$, 
    $\boldsymbol{\xi}\in\mathbb{R}^K$, $\eta$}    
\KwOut{$\mathbf{W}\in\mathbb{R}^{K\times M}$}
...
\Fn{\UpdateMSOM{$\mathbf{X}$, $\mathbf{W}$, $M$, 
	$K$, $\boldsymbol{\xi}$, $\eta$}}{
    ...
    $\overline{\xi} = \frac{\sum_k \xi_k}{K}$
    \tcp*[r]{Modulation mean}\;
    
    \For{$n\leftarrow0$ \KwTo {$N$}}{
    	$r_{n}\leftarrow \argmin_k ||\mathbf{x}_n - \mathbf{w}_k||$
        \tcp*[r]{Winning prototype}
        
        \For{$k\leftarrow1$ \KwTo {$K$}}{
        	$\phi_{k, n}\leftarrow \overline{\xi}\, 
            exp(\gamma_k(k - r_{n})^2)$
            \tcp*[r]{\parbox[t]{2.2in}{\raggedleft 
            Radial bases of distances from winner}}
...
\Fn{\Int{$\mathbf{x}$, $K$, $N$, $\beta$}}{
    ...
    \For{$k=1$ \KwTo {K}}{
    	\For{$i=1$ \KwTo {N}}{
 	    	$\psi_{k, i}(x_i) \leftarrow (\beta\sqrt{2\pi})^{-1}
        	exp{\left(-(0.5((k - x_i) \beta^{-1})^2\right)}$
        \tcp*[f]{Radial bases}
     	}
  	}
...
\Fn{\GenerateFromMSOM{$\mathbf{W}$, $K$, $N$, 
    ...
    \Int{$\mathbf{s}$, $K$, $N$, $\beta$}$
    \tcp*[r]{\parbox[t]{2.2in}{\raggedleft 
    Interpolating samples to get MSOM outputs}}
   
   	$\boldsymbol{X}_{gen} \leftarrow 
    \left(\mathbf{W}^T\boldsymbol{\Psi}\right)^T$
    \tcp*[r]{Back-propagate to the input space}
...
\Fn{\Predict{$\mathbf{X}$, $\mathbf{w}$, $\alpha$}}{
	...
	$\mathbf{y}\leftarrow ({1+exp({-\alpha(\mathbf{X}
    \mathbf{w}-0.5)}))^{-1}}$\; 
   	\Return{$\mathbf{y}$}
}
...
\Fn{\UpdatePredictor{$\mathbf{X}$, $\boldsymbol{\lambda}$, 
    ...
    $\mathbf{w} \leftarrow \mathbf{w} + \eta\gamma\alpha
...
\Begin{
    ...
    \lFor{$i=1$ \KwTo $K$}{$c_i\leftarrow i$}
        $\mathbf{\Psi_{pred}}\leftarrow\Int{$\mathbf{c}$, 
        	$K$, $K$, $\beta$}$\;
    ...
    \BlankLine
 	...
 	\Repeat{$\frac{1}{K}\sum_{i=1}^K\boldsymbol{\xi} = 1$ }{
        ...
        \tcp{Update weights}
        $\mathbf{W}_e\,\leftarrow$\,\UpdateMSOM{$\mathbf{s}$, 
        	$\mathbf{W}_e$, $M$, $K$, $\boldsymbol{\omega}$, $\eta$}\;
        $\mathbf{W}_m\,\leftarrow$\,\UpdateMSOM{$\mathbf{a}$, 
        	$\mathbf{W}_e$, $M$, $K$, $\boldsymbol{\omega}$, $\eta$}\;
...
\Begin{
	...
	$\mathbf{\Psi_{pred}}\leftarrow\Int{$\mathbf{c}$, 
    	$K$, $K$, $\beta$}$\;
    $\mathbf{\Psi_{grid}}\leftarrow\Int{$\mathbf{c}$, 
    	$K$, $K$, $\kappa$}$
...
\Begin{
	...
	\Repeat{$\frac{1}{K}\sum_{i=1}^K\boldsymbol{\xi} = 1$ }{
       ...
       \BlankLine
        ...
        $\boldsymbol{\mu_{\xi}}\leftarrow\boldsymbol{\mu_{\xi}} + 
        	\left(1-\alpha\right) + \alpha\left
            [\boldsymbol{\xi} - \boldsymbol{\xi_{pred}}\right]^{+}$\;
        $i_{\xi} \leftarrow \argmax\,\boldsymbol{\mu_{\xi}}$\;
    	...
    	$\mathbf{p_g}, \mathbf{A},\mathbf{G}\leftarrow$\,
        	\GenerateFromMSOM{$\mathbf{W}_m$, $K$, $N$, 
            $\boldsymbol{\psi_{grid}}_{i_{\xi}}$, $\beta$}\;  
        $\boldsymbol{\xi}_{g}\leftarrow\Predict{$\mathbf{G}$, 
        	$\mathbf{w}_p$, $\alpha$}$\;
        $\boldsymbol{\omega}_{g}\leftarrow 1 - \boldsymbol{\xi}_{g}$\;
        $\mathbf{a}\leftarrow\mathbf{a} + \mathcal{N}(\mathbf{0},
        ...
        \tcp{Update weights}
        $\mathbf{W}_e\,\leftarrow$\,\UpdateMSOM{$\mathbf{s}$, 
        	$\mathbf{W}_e$, $M$, $K$, $\boldsymbol{\omega\odot\mathbf{r}}$,
            $\eta$}\;
        $\mathbf{W}_m\,\leftarrow$\,\UpdateMSOM{$\mathbf{a}$, 
        	$\mathbf{W}_e$, $M$, $K$, $\boldsymbol{\omega}\odot\mathbf{r}$, 
            $\eta$}\;       
